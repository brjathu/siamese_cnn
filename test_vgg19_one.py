    ...
    for l in class_list:
        ...
        for i in range(train_data.shape[1]):
            ...
            if(count % batch_size == 0):
                img1 = utils.load_image("/flush1/raj034/wikiart/wikiart/" + train_data[class_num, i])
            ...
            if ((count + 1) % batch_size == 0):
                ...
                for i in range(batch_size):
                    ...
                    # logEntry("debugger 1 ========= >  " + str(features.shape))

                    gram = np.matmul(features.T, features) / features.size
                    gram = np.array(gram[np.triu_indices(512)])
                    train_vectors.append(gram)
                    train_label.append([train_data[class_num, i], l])
